...
export interface CommandOutputEvent { type:"stdout"|"stderr"; data:Uint8Array }
export type CommandTerminalEvent = { type:"exit"; exitCode:number } | { type:"error"; code:string; message:string; requestId:string };
export class CailSandboxError extends Error { constructor(readonly code:string,message:string,readonly status:number,readonly type:string="unknown_error",readonly param:string|null=null,readonly details:Record<string,unknown>={}){super(message);this.name="CailSandboxError";Object.setPrototypeOf(this,CailSandboxError.prototype)} }
export type FetchLike = (input: RequestInfo | URL, init?: RequestInit) => Promise<Response>;
export interface SandboxClientOptions { baseUrl:string; app:string; fetchImpl?:FetchLike }
const APP=/^[a-z0-9][a-z0-9-]{0,63}$/;
async function parseError(r:Response){let b:any;try{b=await r.json()}catch{return new CailSandboxError("unknown_error",`Sandbox request failed with HTTP ${r.status}.`,r.status)}const e=b?.error;return new CailSandboxError(typeof e?.code==="string"?e.code:"unknown_error",typeof e?.message==="string"?e.message:`Sandbox request failed with HTTP ${r.status}.`,r.status,typeof e?.type==="string"?e.type:"unknown_error",typeof e?.param==="string"?e.param:null,e?.cail&&typeof e.cail==="object"?e.cail:{})}
export function createCailSandboxClient(options:SandboxClientOptions){if(!options.baseUrl.startsWith("https://")&&!options.baseUrl.startsWith("http://localhost"))throw new Error("baseUrl must use HTTPS (or localhost)");if(!APP.test(options.app))throw new Error("app must be a stable lowercase slug");const f=options.fetchImpl??fetch,base=options.baseUrl.replace(/\/+$/,"");
 const call=async(path:string,init:RequestInit,credential:CailSandboxCredential)=>{const h=new Headers(init.headers);h.delete("x-cail-identity-jwt");h.set("x-cail-app",options.app);if(credential.kind==="jwt"){h.delete("authorization");h.set("x-cail-identity-jwt",credential.token)}else{h.delete("x-cail-identity-jwt");h.set("authorization",`Bearer ${credential.token}`)}const r=await f(`${base}${path}`,{...init,headers:h,redirect:"error"});if(!r.ok)throw await parseError(r);return r};
